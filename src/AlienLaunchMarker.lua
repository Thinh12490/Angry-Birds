...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if self.alien.canUseSkill then 
            if love.keyboard.wasPressed('space') then
                local alienCopy1 = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() - 20, 'Player')
                table.insert(self.aliens, alienCopy1)

                -- apply the difference between current X,Y and base X,Y as launch vector impulse
                alienCopy1.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)

                -- make the alien pretty bouncy
                alienCopy1.fixture:setRestitution(0.4)
                alienCopy1.body:setAngularDamping(1)

                local alienCopy2 = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY() + 20, 'Player')
                table.insert(self.aliens, alienCopy2)

                -- apply the difference between current X,Y and base X,Y as launch vector impulse
                alienCopy2.body:setLinearVelocity((self.baseX - self.shiftedX) * 10, (self.baseY - self.shiftedY) * 10)

                -- make the alien pretty bouncy
                alienCopy2.fixture:setRestitution(0.4)
                alienCopy2.body:setAngularDamping(1)

                self.alien.canUseSkill = false
            end
        end
    end
end
